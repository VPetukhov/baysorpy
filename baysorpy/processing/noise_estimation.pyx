...
def _cython_bincount(cnp.int64_t[:] arr, int64_t max_value):
...
def _split(cnp.ndarray array, cnp.ndarray[cnp.int64_t, ndim=1] factor, int max_factor=-1, bint drop_zero=False):
    ...
    counts = _cython_bincount(factor, max_value)
    
    splitted = [np.empty(c, dtype=array.dtype) for c in counts]

    last_id = <int64_t *>malloc((max_value + 1) * sizeof(int64_t))
    for i in range(max_value + 1):
        last_id[i] = 0
...
def _convert_edge_list_to_adj_list(cnp.ndarray[cnp.int_t, ndim=2] edge_list, 
    ...
    cdef list res_ids
    cdef list res_weights
    s1 = split(edge_list[1, :], edge_list[0, :], np.max(edge_list))
    s2 = split(edge_list[0, :], edge_list[1, :], np.max(edge_list))

    res_ids = [np.concatenate([x,y]) for (x, y) in zip(s1, s2)]#np.concatenate((_split(edge_list[1], edge_list[0], n_verts),
              #          _split(edge_list[0], edge_list[1], n_verts)), dtype='object')

    if edge_weights is None:
        return res_ids
    s1 = split(edge_weights, edge_list[0], np.max(edge_list))
    s2 = split(edge_weights, edge_list[1], np.max(edge_list))
    res_weights = [np.concatenate([x,y]) for (x, y) in zip(s1, s2)]
    #np.concatenate((_split(edge_weights, edge_list[0], n_verts),
    #                     _split(edge_weights, edge_list[1], n_verts)), dtype='object')
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef void _wmean_std(cnp.float64_t[:] values, cnp.float64_t[:] weights, cnp.float64_t[:] result) nogil:
...
cdef inline void maximize_noise_distributions(cnp.float64_t[:] edge_lengths, cnp.float64_t[:,:] assignment_probs, cnp.float64_t[:, :] result):
    ...
    for i in range(2):
        _wmean_std(edge_lengths, assignment_probs[:,i], temp_result)
...
def wmean_std(values, weights, result):
    return _wmean_std(values, weights, result)

def adjacency_list(points):
    return _adjacency_list(points)
def cython_bincount(values, max_factor=None):
    return _cython_bincount(values, max_factor)
...
def build_molecule_graph(df_spatial):
    if type(df_spatial) == np.ndarray:
        return _build_molecule_graph(df_spatial)
    else:
        pos_data = position_data(df_spatial).T
    return _build_molecule_graph(pos_data)
